...
class _AgentMissionsScreenState extends State<AgentMissionsScreen> {
  ...
  List<ReportModel> _filtered(List<ReportModel> all) {
    if (_filterIndex == 0) return all.where((r) => r.isActive).toList();
    return all.where((r) => r.isResolved || r.isClosed).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    final filters = [l10n.agentFilterToDo, l10n.agentFilterDone];
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(l10n.agentMissionsTitle),
      ),
      body: Column(
        children: [
          SizedBox(
                    ...
                    : filtered.isEmpty
                        ? Center(
                            child: Column(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                Icon(PhosphorIconsDuotone.clipboardText, size: 56, color: p.textHint.withValues(alpha: 0.5)),
                                const SizedBox(height: 12),
                                Text(l10n.agentNoMissions, style: TextStyle(fontSize: 15, color: p.textHint)),
                              ],
                            ),
                          )
                        : RefreshIndicator(
                            ...
                            child: ListView.builder(
                              ...
                              itemBuilder: (_, i) => _MissionCard(
                                report: filtered[i],
                                provider: provider,
                                onTap: () => context.go('/agent/missions/${filtered[i].id}'),
                                index: i,
                              ),
                            ),
                          ),
...
class _MissionCard extends StatelessWidget {
  const _MissionCard({required this.report, required this.provider, required this.onTap, this.index = 0});
