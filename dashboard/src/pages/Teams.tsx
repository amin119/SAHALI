...
export default function Teams() {
    ...
    Promise.all([
      ...
      .then(([users, rData, munis]) => {
        ...
        users.forEach(u => {
          ...
          const res = mine.filter(r => r.status === 'resolved' || r.status === 'closed').length
          const inp = mine.filter(r => r.status === 'in_progress' || r.status === 'scheduled').length
          map[u.id] = { assigned: mine.length, resolved: res, inProgress: inp, performance: mine.length > 0 ? Math.round((res / mine.length) * 100) : 0 }
        })
        setStatsMap(map)
      })
  ...
  async function toggleActive(user: User) {
    ...
    try {
      ...
      if (editUser?.id === user.id) setEditUser(prev => prev ? { ...prev, is_active: !prev.is_active } : null)
    } catch (e: unknown) {
    ...
    <div>
      ...
      <div className="flex gap-6">
        ...
        {detail && (() => {
          ...
          const pc = perfColor(s.performance)
          return (
            <div className="w-72 flex-shrink-0 bg-white rounded-xl border border-[#E2E8F0] shadow-sm h-fit sticky top-24">
              ...
              <div className="p-5">
                ...
                {detail.email && <div className="flex items-center gap-2 text-sm text-[#64748B] mb-2"><span className="material-symbols-outlined" style={{ fontSize: 16 }}>email</span><span className="truncate">{detail.email}</span></div>}
